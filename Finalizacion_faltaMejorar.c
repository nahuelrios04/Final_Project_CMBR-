...
static const char *TAG = "ROBOT_HYBRID_FINAL_V6";
...
// --- Constantes de Homing Eje 1 (Para PWM) ---
...
// --- Constantes de Homing Ejes 2 y 3 (Para PWM) ---
...
static void eje2_wait_hall_low(void) { int c=0; for(;;){ if(check_sensor(PIN_HALL_2)){ if(++c>=2)return; }else c=0; vTaskDelay(1); } }
static void eje2_wait_hall_high(void){ int c=0; for(;;){ if(!check_sensor(PIN_HALL_2)){ if(++c>=2)return; }else c=0; vTaskDelay(1); } }
static void eje3_wait_hall_low(void) { int c=0; for(;;){ if(check_sensor(PIN_HALL_3)){ if(++c>=2)return; }else c=0; vTaskDelay(1); } }
static void eje3_wait_hall_high(void){ int c=0; for(;;){ if(!check_sensor(PIN_HALL_3)){ if(++c>=2)return; }else c=0; vTaskDelay(1); } }
...
/* ============================================================
 * LÓGICA HOMING (USA PWM/LEDC COMPLEJO)
 * ============================================================ */

typedef enum { SOBREPASO_OK=0, SOBREPASO_ENDSTOP=1 } sobrepaso_res_t;

// --- EJE 1 Homing (con Rampas y Espera) ---
static void homing_eje1_logic(void) {
    ESP_LOGI(TAG, "[EJE1] Homing secuencial (PWM)...");
    // Configurar STEP a LEDC
    ...
    // Pass 2: Retorno y Alineación Lenta
    if (check_sensor(PIN_HALL_1)) {
...
// --- Eje 2 Homing (con Rampas y Verificación) ---
static sobrepaso_res_t eje2_check_over(void) {
    ...
    while(1) {
        if(check_sensor(PIN_HALL_2)){ if(++cl>=2){ eje2_pwm_stop(); return SOBREPASO_ENDSTOP; } } else cl=0;
...
static bool eje2_start_on_hall(int *d) {
    ...
    *d=IZQUIERDA_2; eje2_set_dir(*d); if(eje2_check_over()==SOBREPASO_ENDSTOP){eje2_esc_end(d); return false;}
    ...
    gpio_set_level(PIN_LED_2, 1); return true;
}
...
static void homing_eje2_logic(void) {
    ESP_LOGI(TAG, "[EJE2] Homing secuencial (PWM)...");
    // Configurar STEP a LEDC
...
// --- Eje 3 Homing (con Rampas y Verificación) ---
static sobrepaso_res_t eje3_check_over(void) {
    ...
    int64_t t0 = esp_timer_get_time(); int cl=0;
    while(1) {
        if(check_sensor(PIN_HALL_3)){ if(++cl>=2){ eje3_pwm_stop(); return SOBREPASO_ENDSTOP; } } else cl=0;
        if(((esp_timer_get_time()-t0)/1000) >= TIEMPO_SOBREPASO_MS_3) { eje3_pwm_stop(); return SOBREPASO_OK; }
        vTaskDelay(1);
    }
}
static void eje3_esc_end(int *d) {
    // Nota: PIN_HALL_3 y PIN_LED_3 comparten el pin 4. No controlamos LED aquí.
...
static void homing_eje3_logic(void) {
    ESP_LOGI(TAG, "[EJE3] Homing secuencial (PWM)...");
    // Configurar STEP a LEDC
...
static void ptp_sequence_task(void *arg) {
    ...
    // 1. Asegurar que los pines STEP son GPIO OUTPUT (Desconecta el LEDC si estaba activo)
