...
class RobotUI(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        speed_box = QtWidgets.QGroupBox("Velocidad Jog/PTP")
        speed_layout = QtWidgets.QHBoxLayout()
        self.lbl_speed = QtWidgets.QLabel(f"{self.current_freq} Hz")
        self.lbl_speed.setFixedWidth(60)
        
        self.slider_speed = QtWidgets.QSlider(QtCore.Qt.Horizontal)
        self.slider_speed.setRange(10, 100) # 10% a 100%
        self.slider_speed.setValue(40)      # ~1000Hz
        
        speed_layout.addWidget(QtWidgets.QLabel("Lento"))
        speed_layout.addWidget(self.slider_speed)
        speed_layout.addWidget(QtWidgets.QLabel("Rápido"))
        ...
        # Enviar velocidad inicial despues de un momento
        QtCore.QTimer.singleShot(2000, self.send_speed_cmd)

    # --- VELOCIDAD ---
    def update_speed_label(self):
        val = self.slider_speed.value()
        # 10 -> 200Hz, 100 -> 2500Hz
        freq = int(200 + (val - 10) * (2300 / 90))
        self.lbl_speed.setText(f"{freq} Hz")
        self.current_freq = freq

    def send_speed_cmd(self):
        self.send_spi(700, self.current_freq, 0)
        self.log_cmd(f"Velocidad enviada: {self.current_freq} Hz")
    ...
    def send_spi(self, cmd, a1, a2):
        ...
        try:
            ...
            if cmd != 0 and cmd != 700: self.log_cmd(f"TX: {cmd} {a1} {a2}")
        except OSError as e:
    ...
    def _open_preset_dialog(self):
        ...
        if run_dialog(dlg) == QtWidgets.QDialog.Accepted:
            ...
            if abs(px) > 1000 or "P1" in name or "P2" in name or "P7" in name: 
                self.log_cmd(f"PTP Directo (Steps): {name}")
                t1 = px / PASOS_POR_GRADO_1
                t2 = py / PASOS_POR_GRADO_2
                t3 = pz / PASOS_POR_GRADO_3
                robot.theta1, robot.theta2, robot.theta3 = t1, t2, t3
                self.update_labels()
                self.send_spi(400, int(px), int(py))
                time.sleep(0.002)
                self.send_spi(401, int(pz), 0)
            else:
                self.log_cmd(f"PTP Objetivo (XYZ): {name}")
    ...
    def handle_status(self, s):
        if not s: return
        if time.time() < self.ignore_status_until: return
