...
# ==========================================
# ==== CONFIGURACIÓN DE PASOS (CORREGIDA) ====
# ==========================================
...
REDUCCION_EJE_1 = 12.0  # <--- DATO NUEVO: 12:1
REDUCCION_EJE_2 = 16.0   # Directo (Asumido, cambiar si tiene reducción)
REDUCCION_EJE_3 = 16.0   # Directo (Asumido, cambiar si tiene reducción)

# CÁLCULO DE RATIOS (Pasos por Grado)
# Fórmula: (Motor * Microsteps * Reduccion) / 360
...
# Definición de Puntos (Ahora calculados con el nuevo Ratio)
# Usamos coordenadas angulares para P1-P6 para verificar la reducción
...
class RobotUI(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Control Robot - Secuenciador (Ratio 12:1)")
        ...
        self.slider.setRange(200, 5000); self.slider.setValue(1000) # Aumentado max a 5000Hz para el eje 12:1
    ...
    def start_sequence(self):
        ...
        for idx in points_indices:
            ...
            if "P1" in name or "P2" in name or "P3" in name or "P4" in name:
                # Ya son pasos directos calculados arriba
                t1, t2, t3 = px, py, pz
            else:
                res = robot.calculate_ik(px, py, pz)
                if res: 
                    # Convertir grados IK a pasos
                    a1, a2, a3 = res
                    # Nota: Aquí calculamos la posición absoluta en PASOS
                    # Pero la función send_kinematic_move espera Ángulos en grados.
                    # Así que guardamos grados para la cola si es IK.
                    t1, t2, t3 = a1, a2, a3 
                else: 
                    self.log_cmd(f"Error IK en {name}"); continue
            
            # Truco: Si el nombre tiene 'P' son pasos, si no, son grados.
    ...
    def process_next_step(self):
        ...
        if is_steps:
            # t1, t2, t3 son TARGET STEPS absolutos.
            # Debemos convertir esto a DELTA pasos para enviar al ESP32.
            # Pero send_kinematic_move trabaja con GRADOS absolutos.
            # Convertimos Pasos Abs -> Grados Abs
    ...
    def do_home(self):
        ...
        # === RESETEO DE POSICIÓN ABSOLUTA (CRÍTICO) ===
        self.last_a1, self.last_a2, self.last_a3 = 0.0, 0.0, 0.0
        robot.theta1, robot.theta2, robot.theta3 = 0.0, 0.0, 0.0
        self.update_ui()
    ...
    def send_kinematic_move(self, a1, a2, a3):
        # Calcular diferencia angular desde la última posición conocida
        ...
        # Calcular pasos INCREMENTALES para enviar al ESP32
    ...
    def on_status(self, s):
        ...
        if "500" in s:
            ...
            self.lbl_status.setStyleSheet("color:red" if self.is_homing else "color:orange")
